...
// API Configuration
const API_BASE_URL = 'https://nutrient-predictor-backend.onrender.com';
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.status === 503) {
      errorMessage = 'Models not loaded on server. Please ensure backend is properly set up.';
...
export const predictNutrientDeficiencies = async (profile: UserProfile): Promise<PredictionResponse> => {
